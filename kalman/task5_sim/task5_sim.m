...
for i=1:8
    for j=2:simlen
        for k =1:1000
            ...
            st_post(i,j,k)=state(j+simlen*(k-1),i);
        end
        ...
        stmean_post(i,j)=mean(st_post(i,j,:));
    end
end
...
for i=1:8
    ...
    plot(std_post(i,:));
    figure(8+i);
    plot(lin,stmean_post(i,:)-mean_post(i,:),lin,stmean_post(i,:)+mean_post(i,:));
end
